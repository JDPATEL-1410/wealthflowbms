...
import { BrokerageTransaction, TransactionStatus, TransactionSource, TeamMember, Role, ImportBatch, Client, MappingEntry } from '../types';
...
export const Imports: React.FC<ImportsProps> = ({ currentUser }) => {
  ...
  // Track new mappings discovered in the current file
  const [discoveredAmcMappings, setDiscoveredAmcMappings] = useState<MappingEntry[]>([]);
  const [discoveredSchemeMappings, setDiscoveredSchemeMappings] = useState<MappingEntry[]>([]);
  ...
  const processData = () => {
    setProcessing(true);
    setProcessingStep('Standardizing & Validating...');

    setTimeout(() => {
      const transactions: BrokerageTransaction[] = [];
      const foundNewClients: Record<string, Client> = {};
      const newAmcMappings: MappingEntry[] = [];
      const newSchemeMappings: MappingEntry[] = [];

      const existingAmcDict: Record<string, string> = {};
      amcMappings.forEach(m => existingAmcDict[m.original] = m.standard);

      const existingSchemeDict: Record<string, string> = {};
      schemeMappings.forEach(m => existingSchemeDict[m.original] = m.standard);
      ...
      for (let i = headerRowIndex + 1; i < rawFileContent.length; i++) {
        ...
        const rawAmc = String(row[mapIndices['amcName']] || 'Unknown AMC').trim();
        const rawScheme = String(row[mapIndices['schemeName']] || 'Unknown Scheme').trim();

        let standardizedAmc = existingAmcDict[rawAmc] || rawAmc;
        let standardizedScheme = existingSchemeDict[rawScheme] || rawScheme;

        // Track new mappings to be added to the dictionary automatically
        if (!existingAmcDict[rawAmc] && !newAmcMappings.find(m => m.original === rawAmc)) {
          newAmcMappings.push({ original: rawAmc, standard: rawAmc });
        }
        if (!existingSchemeDict[rawScheme] && !newSchemeMappings.find(m => m.original === rawScheme)) {
          newSchemeMappings.push({ original: rawScheme, standard: rawScheme });
        }
        ...
        transactions.push({
          ...
          pan,
          investorName,
          amcName: standardizedAmc,
          schemeName: standardizedScheme,
          category: 'Equity',
          grossAmount: gross,
          currentValue,
          brokerageRate,
      ...
      setDiscoveredAmcMappings(newAmcMappings);
      setDiscoveredSchemeMappings(newSchemeMappings);
  ...
  const handleConfirmImport = () => {
    ...
    // Auto-save new clients
    if (newClientsFound.length > 0) {
      upsertClients(newClientsFound);
    }

    // AUTO-SAVE NEW STANDARDIZATION MAPPINGS
    if (discoveredAmcMappings.length > 0) {
      updateAmcMappings([...amcMappings, ...discoveredAmcMappings]);
    }
    if (discoveredSchemeMappings.length > 0) {
      updateSchemeMappings([...schemeMappings, ...discoveredSchemeMappings]);
    }
    ...
    alert(`Success: ${previewData.length} transactions imported. New AMCs/Schemes were automatically registered in the Standardizer.`);
  ...
  const clearUpload = () => {
    ...
    setPreviewData(null);
    setNewClientsFound([]);
    setDiscoveredAmcMappings([]);
    setDiscoveredSchemeMappings([]);
    setRawFileContent([]);
    setImportStage('UPLOAD');
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Processing brokerage files with automated name standardization.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 space-y-6">
            <div className={`bg-white p-6 rounded-xl border border-slate-200 shadow-sm transition-opacity ${importStage !== 'UPLOAD' ? 'opacity-50 pointer-events-none' : ''}`}>
              <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center"><span className="bg-blue-100 text-blue-600 w-6 h-6 rounded-full inline-flex items-center justify-center text-xs mr-2">1</span>Source & Period</h3>
              ...
              <div className="space-y-3">
                <label className="text-xs font-bold text-slate-500 uppercase flex items-center"><Calendar className="w-3 h-3 mr-1" /> Settlement Month</label>
                <div className="grid grid-cols-2 gap-2">
                  <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="border rounded-lg p-2 text-sm bg-slate-50 focus:ring-2 focus:ring-blue-500 font-bold">
                    {MONTHS.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
                  </select>
                  <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="border rounded-lg p-2 text-sm bg-slate-50 focus:ring-2 focus:ring-blue-500 font-bold">
            ...
            <div className={`bg-white p-6 rounded-xl border border-slate-200 shadow-sm transition-opacity ${importStage !== 'UPLOAD' && importStage !== 'HEADER_SELECTION' ? 'opacity-50' : ''}`}>
              <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center"><span className="bg-blue-100 text-blue-600 w-6 h-6 rounded-full inline-flex items-center justify-center text-xs mr-2">2</span>Select File</h3>
              ...
              {!uploadedFile ? (
                ...
                <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                  ...
                  {processing ? <div className="text-xs text-blue-600 animate-pulse font-black uppercase tracking-widest">{processingStep}</div> : <div className="text-xs text-green-600 font-bold flex items-center uppercase tracking-widest"><CheckCircle className="w-3 h-3 mr-1" /> File Validated</div>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 min-h-[400px]">
            {importStage === 'HEADER_SELECTION' && (
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm h-full flex flex-col overflow-hidden animate-in fade-in slide-in-from-right-4">
                <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                  ...
                  <button onClick={confirmHeaderRow} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-md shadow-blue-50 transition hover:bg-blue-700">Confirm Header Row <ArrowRight className="inline w-4 h-4 ml-1" /></button>
                </div>
            ...
            {importStage === 'MAPPING' && (
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm h-full flex flex-col animate-in fade-in slide-in-from-right-4">
                <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                  ...
                  <button onClick={processData} className="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm font-medium shadow-md shadow-blue-50 transition hover:bg-blue-700">Standardize Data <ArrowRightCircle className="inline w-4 h-4 ml-1" /></button>
                </div>
                <div className="p-6 space-y-4 overflow-y-auto">
                  {SYSTEM_FIELDS.map(field => (
                    <div key={field.key} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition">
                      <span className="text-sm font-bold text-slate-700">{field.label} {field.required && <span className="text-red-500">*</span>}</span>
                      <select value={columnMapping[field.key] || ''} onChange={(e) => setColumnMapping(prev => ({ ...prev, [field.key]: e.target.value }))} className="w-1/2 border rounded-lg px-3 py-2 text-sm bg-white font-medium focus:ring-2 focus:ring-blue-500">
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm h-full flex flex-col overflow-hidden animate-in fade-in slide-in-from-right-4">
                <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{previewData.length} records • {autoMapStats.mapped} mapped to clients.</p>
                  </div>
                  <button onClick={handleConfirmImport} className="bg-green-600 text-white px-6 py-2 rounded-lg text-sm font-bold shadow-lg shadow-green-50 hover:bg-green-700 transition">Finalize Import</button>
                </div>
                <div className="bg-blue-50 border-b border-blue-100 p-3 text-[10px] text-blue-700 font-bold uppercase tracking-widest flex items-center">
                  <RefreshCw className="w-3 h-3 mr-2 animate-spin-slow" /> Standard names automatically applied from dictionary
                </div>
                <div className="overflow-auto flex-1">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-white sticky top-0 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="px-4 py-3">Standard Scheme</th>
                        ...
                        <tr key={row.id} className="hover:bg-slate-50">
                          <td className="px-4 py-3">
                            <div className="flex flex-col">
                              <span className="font-bold text-slate-900 truncate max-w-[150px]">{row.investorName}</span>
                              <span className="text-[10px] text-blue-600 uppercase font-black tracking-tighter">{row.amcName}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3 text-slate-500 truncate max-w-[180px] font-medium">{row.schemeName}</td>
            ...
            {importStage === 'UPLOAD' && (
              <div className="h-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center p-10 text-slate-400">
                ...
                <p className="text-sm font-medium text-center">Upload spreadsheet to start automated processing.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-500 font-bold border-b border-slate-200 uppercase tracking-widest text-[10px]">
              <tr>
                <th className="px-6 py-4">Batch File</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {batches.map(batch => (
                <tr key={batch.id} className="hover:bg-slate-50 transition">
                  <td className="px-6 py-4 font-bold text-slate-900">{batch.fileName}</td>
                  <td className="px-6 py-4 text-slate-500 font-medium">{new Date(batch.uploadDate).toLocaleDateString()}</td>
                  <td className="px-6 py-4 text-center text-slate-600 font-bold">{batch.totalLines}</td>
                  <td className="px-6 py-4 text-right font-black text-blue-600">₹ {batch.totalGross.toLocaleString()}</td>
                  <td className="px-6 py-4 text-center">
                    <span className="inline-flex px-3 py-1 rounded-full text-[10px] font-black bg-green-50 text-green-700 border border-green-200 uppercase tracking-widest">Live</span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <button onClick={() => handleDeleteBatch(batch)} className="p-2 text-slate-400 hover:text-red-600 bg-slate-50 rounded-lg transition" title="Delete Batch"><Trash2 className="w-4 h-4" /></button>
                  </td>
                </tr>
              ))}
              {batches.length === 0 && (
                <tr><td colSpan={6} className="py-20 text-center text-slate-400">No import history found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
