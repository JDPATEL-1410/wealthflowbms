...
import { PieChart, Mail, Lock, Key, ArrowRight, ShieldCheck, RefreshCcw, CheckCircle, AlertCircle, Eye, EyeOff } from 'lucide-react';
...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const user = team.find(u => 
        (u.email?.toLowerCase() === email.toLowerCase() || u.code.toLowerCase() === email.toLowerCase()) && 
        u.password === password
      );
  ...
  const handleSendOtp = (e: React.FormEvent) => {
    ...
    if (user) {
      ...
      console.log(`%c [MAIL SIMULATION] OTP for ${email}: ${code} `, 'background: #2563eb; color: #fff; font-weight: bold; padding: 4px;');
      alert(`Simulation: OTP ${code} has been sent to ${email}`);
      setView('OTP');
    } else {
        ...
        <div className="bg-white rounded-3xl shadow-2xl shadow-slate-200 overflow-hidden border border-slate-100">
            ...
            {view === 'FORGOT' && (
              <form onSubmit={handleSendOtp} className="space-y-6">
                <div className="text-center mb-8">
                  ...
                  <p className="text-slate-500 text-sm mt-1">Enter your registered email to receive an OTP</p>
                </div>
                ...
                <div className="flex flex-col space-y-3">
                  <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-100 hover:bg-blue-700 transition">Send OTP Code</button>
                  <button type="button" onClick={() => setView('LOGIN')} className="w-full text-slate-500 font-bold py-2 text-sm hover:text-slate-900 transition">Back to Login</button>
                </div>
              </form>
            )}
            ...
            {view === 'OTP' && (
              <form onSubmit={handleVerifyOtp} className="space-y-6">
                <div className="text-center mb-8">
                  ...
                  <p className="text-slate-500 text-sm mt-1">A 6-digit code has been sent to {email}</p>
                </div>
                ...
                <div className="flex flex-col space-y-3 text-center">
                   <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-100 hover:bg-blue-700 transition">Verify & Proceed</button>
                   <p className="text-xs text-slate-400">Didn't receive it? <button type="button" onClick={handleSendOtp} className="text-blue-600 font-bold">Resend OTP</button></p>
                </div>
              </form>
            )}
            ...
            {view === 'RESET' && (
              <form onSubmit={handleResetPassword} className="space-y-6">
                <div className="text-center mb-8">
                  ...
                  <p className="text-slate-500 text-sm mt-1">Create a new secure password for your account</p>
                </div>
                ...
                <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-100 hover:bg-blue-700 transition">Update Password</button>
              </form>
            )}
          </div>
          ...
          <div className="bg-slate-50 p-4 border-t border-slate-100 text-center">
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center justify-center">
              <ShieldCheck className="w-3 h-3 mr-1.5" /> Managed & Encrypted Enterprise System
            </p>
          </div>
        </div>
      </div>
